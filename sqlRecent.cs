        ...
        public static DataSet Return3Data(string SELECT_COMM_COL, string SELECT_COMM_TABLE, string SELECT_COMM_ORDERBY_DATE, string NO_STRING, string ipno, string edate)
        {
            ...
            for (int i = 1; i < 4; i++)
            {
                if (order.Length != 2)
                {
                    ...
                    cmd = "SELECT * FROM ( SELECT " + SELECT_COMM_COL + ", ROW_NUMBER() OVER (ORDER BY " + SELECT_COMM_ORDERBY_DATE + " DESC) AS ROW FROM " + SELECT_COMM_TABLE + " WHERE " + NO_STRING + " ='" + ipno + "') A WHERE A.ROW BETWEEN " + i + " AND 3";
                }
                else
                {
                    ...
                    cmd = "SELECT * FROM ( SELECT " + SELECT_COMM_COL + ", ROW_NUMBER() OVER (ORDER BY " + order[0] + " DESC , "+order[1]+" DESC) AS ROW FROM " + SELECT_COMM_TABLE + " WHERE " + NO_STRING + " ='" + ipno + "') A WHERE A.ROW BETWEEN " + i + " AND 3";
                }
