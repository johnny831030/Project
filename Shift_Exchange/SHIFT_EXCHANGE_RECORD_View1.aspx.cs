...
{
    public partial class SHIFT_EXCHANGE_RECORD_View1 : System.Web.UI.Page
    {
        ...
        {
            Session.Contents.Remove("tab_state_SHIFT_EXCHANGE_RECORD_View1");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["ipno"] != null)
            {
                ...
                //只有查詢權限
                //if(權限session==only query && Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString()=="10")
                //{ 
                //Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"] = "11"; 
                //}
                //若有新增權限
                //else if(權限session==insert && Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString()=="10")
                //{ 
                Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"] = "12";
                //}
                //else
                //{
                //Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"] = "10";
                ...
                if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "10")
                {
                    State0();//最原始狀態
                }
                if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "11")
                {
                    State1();//只有查詢權限時進入狀態1的按鈕狀態程序
                }
                if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "12")
                {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_lastrecord = sqlShiftExchange.SearchLastSERecordNR(ipno, user, connection_id, sdate, edate);
            if (Convert.ToInt32(dt_lastrecord.Rows.Count.ToString()) <= 0)
            {
                ...
                lblShowNoSERecord.Text = "此住民目前無最近護理交班紀錄";
            ...
            else
            {
                ...
                lblSEContent.Text = dt_lastrecord.Rows[0][2].ToString().Replace("<br />", "\n");
                lblSEContent.Text = lblSEContent.Text.Replace("&nbsp;", " ");
                lblSEContent.Text = SpecialCharactersTransform.strtosc(lblSEContent.Text);
                lblSEContent.Text = "內容：" + lblSEContent.Text;
            }
        ...
        {
            DataTable dt_ditorecord = sqlShiftExchange.SearchDitoSERecordNR(sstr_ipno.Text, sstr_hid.Text);
            if (Convert.ToInt32(dt_ditorecord.Rows.Count.ToString()) <= 0)
            {
                ...
                //ScriptManager.RegisterClientScriptBlock(UpdatePanel12, typeof(UpdatePanel), "test", "alert('" + "此住民目前無護理交班紀錄" + "');location.href='SHIFT_EXCHANGE_RECORD_View1.aspx';", true);
            }
            else
            {
        ...
        {
            Response.Redirect("../../NullReport/護理交班紀錄.docx");
            /*
            ...
            "<div><table style=\"font-weight: bold; font-size:larger; text-align: center; width: 18cm\"><tr><td>護理交班紀錄</td></tr></table></div>" +
        ...
        {
            Response.Redirect("SHIFT_EXCHANGE_RECORD_N.docx");
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        string warning = sqlShiftExchange.addShiftExchangeNR(sstr_ipno.Text, content, create_date, create_time, create_user, sstr_hid.Text);
                        if (warning == "新增失敗")
                        {
                        ...
                        else
                        {
                            ...
                            State3();
                            Session["tab_state_temp"] = Session["tab_state"].ToString() + "_SHIFT_EXCHANGE_RECORD_View1";
                        }
                        //cleanControl();
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt_ip_thisserecord = sqlShiftExchange.SearchIPThisSERecordNR(sstr_ipno.Text, sstr_account.Text, nowdate, sstr_hid.Text);
                ...
                string r_user = sqlShiftExchange.SearchA_user(dt_ip_thisserecord.Rows[0][8].ToString(), sstr_hid.Text);
                ...
                string p_user = sqlShiftExchange.SearchA_user(sstr_account.Text, sstr_hid.Text);
                DataTable dt_ip_info = sqlShiftExchange.SearchIPInfo(sstr_ipno.Text, sstr_hid.Text);
                ...
                //設定頁首頁尾
                //宣告檔案名稱
                string pdffilename = "NR_ShiftExchRecord_" + sqlTransToIP_NO_NEW.getIP_NO_NEW(sstr_hid.Text, sstr_ipno.Text) + ".pdf";
                ...
                info[1] = new PdfPCell(new iTextSharp.text.Phrase("家字號：" + sqlTransToIP_NO_NEW.getIP_NO_NEW(sstr_hid.Text, sstr_ipno.Text), ChFont));
                ...
                Doc.Close();
                Response.Clear();
                //Response.AddHeader("Content-Disposition", "attachment; filename=ShiftExchangeRecord_" + sqlTransToIP_NO_NEW.getIP_NO_NEW(sstr_hid.Text, sstr_ipno.Text) + ".pdf");
                Response.AddHeader("Content-Disposition", "inline; filename=ShiftExchangeRecord_" + sqlTransToIP_NO_NEW.getIP_NO_NEW(sstr_hid.Text, sstr_ipno.Text) + ".pdf");
        ...
        protected void LinkBtnGoToUView_Click(object sender, EventArgs e)
        {
            ...
            Session["tab_state_EXCHANGE_RECORD_View2"] = "20";
        ...
        {
            Session.Contents.Remove("tab_state_SHIFT_EXCHANGE_RECORD_View3");
            Session["tab_state_EXCHANGE_RECORD_View3"] = "20";
            Response.Redirect("SHIFT_EXCHANGE_RECORD_View3.aspx");
        }
        protected void LinkBtnGoToMQView_Click(object sender, EventArgs e)
        {
            Session.Contents.Remove("tab_state_SHIFT_EXCHANGE_RECORD_View1");
            Session["tab_state_EXCHANGE_RECORD_View4"] = "30";
        ...
        protected void ibtnPhraseShiftExchangeContent_Click(object sender, ImageClickEventArgs e)
        {
            ...
            string pharsename = sqlPHRASE.getphrase("護理交班-內容");
            int selectvalue = 1;
            TabContainerPHRASE.Visible = true;
            sqlPHRASE.searchphrase(pharsename,"護理交班-內容");
        ...
        {
            if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"] != null)
            {
                if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "10")
                {
                    State0();//最原始狀態
                }
                else if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "11")
                {
                    State1();//只有查詢權限時進入狀態1的按鈕狀態程序
                }
                else if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "12")
                {
                    State2();//有新增權限時進入狀態2的按鈕狀態程序
                }
                else if (Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"].ToString() == "13")
                {
            ...
            {
                Session["tab_state_SHIFT_EXCHANGE_RECORD_View1"] = "";
            }

            
            if (Session["tab_state_temp"] != null)
            {
                if (Session["tab_state_temp"].ToString() == "10_SHIFT_EXCHANGE_RECORD_View1")
                {
                    Session["tab_state"] = "10";
                    State0();//最原始狀態
                }
                else if (Session["tab_state_temp"].ToString() == "11_SHIFT_EXCHANGE_RECORD_View1")
                {
                    Session["tab_state"] = "11";
                    State1();//只有查詢權限時進入狀態1的按鈕狀態程序
                }
                else if (Session["tab_state_temp"].ToString() == "12_SHIFT_EXCHANGE_RECORD_View1")
                {
                    Session["tab_state"] = "12";
                    State2();//有新增權限時進入狀態2的按鈕狀態程序
                }
                else if (Session["tab_state_temp"].ToString() == "13_SHIFT_EXCHANGE_RECORD_View1")
                {
                    Session["tab_state"] = "13";
                    State3();//新增後進入狀態3的按鈕狀態程序
                }
        ...
        protected void btnNRS_ShiftRec_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_shift_exchange = sqlShiftExchange.getShiftExchange(s_date, e_date, ip_no, sstr_hid.Text);
        ...
        protected void btn_Click(object sender, EventArgs e)
        {
            ...
            string s_date = sqlTime.DateSplitSlash(txtS_DATE.Text);
            string e_date = sqlTime.DateSplitSlash(txtE_DATE.Text);
            DataTable dt_shift_exchange = sqlShiftExchange.getShiftExchange_nonNRS(s_date, e_date, ip_no, sstr_hid.Text);
            GridView1.DataSource = BindGridData(dt_shift_exchange, ip_no);
            GridView1.DataBind();
        ...
        protected void btnModify_shift_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_shift_exchange = sqlShiftExchange.getALLShiftExchange(s_date, e_date, sstr_hid.Text, ip_no);
